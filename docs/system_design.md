...
- **デプロイ・開発基盤:**
  ...
  - ローカル開発環境として Docker (`node:22-bookworm` ベース) を提供し、`docker compose up` でD1データベースを含む環境を即座に構築可能。
...
├── compose.yaml              # Docker Compose設定情報（ローカル開発用）
├── Dockerfile.dev            # ローカル開発用Dockerイメージファイル
...
## 7. セキュリティ・保護対策
本プロジェクトでは安全に運用するため以下の対策を実施しています。
- **SQLインジェクション対策**: 全ての D1 クエリに対してプレースホルダ (`.bind()`) を適用。
- **XSS（クロスサイトスクリプティング）対策**: React のデフォルトエスケープ機構に依存し、`dangerouslySetInnerHTML` は排除。
- **DoS / 長大ペイロード攻撃**: `/contact` API等において、フィールド検証だけでなく文字数制限（バリデーション）を実施。
- **Webhook Markdown インジェクション**: Discord 等に通知を飛ばす際、本文のバッククォートを無害化する処理を実装。
- **ブルートフォース（総当たり攻撃）**: 管理者ページログインに対する Rate Limiting（Cloudflare WAFによる保護）を推奨構成とする。
